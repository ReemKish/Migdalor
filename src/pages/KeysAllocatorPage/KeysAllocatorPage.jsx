...
const KeysAllocator = () => {
    ...
    // Helper to check if lesson belongs to user's Gdud
    ...
    const allocateKeys = async () => {
        ...
        // Fetch previous day logic... (שמור ללא שינוי מהקוד המקורי, רק מקוצר כאן לנוחות)
        ...
        <Container maxWidth="xl" sx={{ py: 6 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: { xs: "column", lg: "row" }, justifyContent: "space-between", gap: 2, mb: 4 }}>
                ...
                <Box sx={{ display: "flex", gap: 1.5 }}>
                    <Button
                        variant="contained"
                        startIcon={isAllocating ? <CircularProgress size={16} color="inherit" /> : <Wand2 size={18} />}
                        onClick={allocateKeys}
                        disabled={selectedKeys.length === 0 || pendingCount === 0 || isAllocating}
                        sx={{ bgcolor: "#10b981", borderRadius: '12px', fontWeight: 600, '&:hover': { bgcolor: "#059669" } }}
                    >
                    ...
                    <Button
                        ...
                        sx={{ borderRadius: '12px', color: isDark ? '#94a3b8' : 'inherit', borderColor: isDark ? 'rgba(255,255,255,0.2)' : 'inherit' }}
                    >
                        אפס
                    </Button>
                    <Button
                        ...
                        sx={{ color: "#f87171", borderColor: isDark ? 'rgba(248, 113, 113, 0.3)' : "#f87171", borderRadius: '12px', '&:hover': { bgcolor: 'rgba(248, 113, 113, 0.1)', borderColor: '#f87171' } }}
                    >
                        מחק הכל
                    </Button>
                </Box>
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
                {[
                    ...
                    { label: 'מפתחות זמינים', value: selectedKeys.length, color: '#3b82f6', bg: isDark ? 'rgba(59, 130, 246, 0.1)' : '#eff6ff' },
                    ...
                    <Card sx={{
                        ...
                        <Box sx={{ p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid #f1f5f9' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Typography variant="h6" sx={{ color: isDark ? 'white' : '#1e293b', fontWeight: 600 }}>מפתחות זמינים</Typography>
                            </Box>
                            <Button size="small" onClick={toggleSelectAll} sx={{ borderRadius: '8px' }}>
                                {selectedKeys.length === allKeys.length ? "בטל הכל" : "בחר הכל"}
                            </Button>
                        </Box>
                        <Box sx={{ maxHeight: '600px', overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                            {allKeys.map(key => (
                                <Paper key={key.id} elevation={0} sx={{
                                    ...
                                    <Checkbox
                                        ...
                                        sx={{ color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)', '&.Mui-checked': { color: '#10b981' } }}
                                    />
                                    <Box sx={{ flex: 1 }}>
                                        <Typography variant="body2" sx={{ fontWeight: 600, color: isDark ? 'white' : '#334155' }}>חדר {key.room_number}</Typography>
                                        <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                            <Chip
                                                icon={key.room_type === "פלוגתי" ? <Briefcase size={12} /> : <Home size={12} />}
                                                label={key.room_type}
                                                size="small"
                                                sx={{ height: 20, fontSize: '0.7rem', bgcolor: isDark ? 'rgba(255,255,255,0.1)' : '#e2e8f0' }}
                                            />
                                            <Chip
                                                ...
                                                sx={{ height: 20, fontSize: '0.7rem', bgcolor: isDark ? 'rgba(255,255,255,0.1)' : '#e2e8f0' }}
                                            />
                                            {key.has_computers && <Chip icon={<Computer size={12} />} label="מחשב" size="small" sx={{ height: 20, fontSize: '0.7rem', bgcolor: '#dbeafe', color: '#1e40af' }} />}
                                        </Box>
                                    </Box>
                                </Paper>
                            ))}
                                    ...
                                    {isLoading ? (
                                        ...
                                        allItemsToDisplay.map(lesson => (
                                            <TableRow key={lesson.id} sx={{ '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.03)' : '#f8fafc' } }}>
                                                ...
                                                <TableCell align="center" sx={cellStyle}>
                                                    <Chip
                                                        label={lesson.room_type_name === "פלוגתי" ? "🏢 פלוגתי" : "🏠 צוותי"}
                                                        size="small"
                                                        sx={{
                                                            bgcolor: lesson.room_type_name === "פלוגתי" ? (isDark ? 'rgba(139, 92, 246, 0.15)' : '#f3e8ff') : (isDark ? 'rgba(59, 130, 246, 0.15)' : '#dbeafe'),
                                                            color: lesson.room_type_name === "פלוגתי" ? '#a855f7' : '#2563eb',
                                                            fontSize: '0.75rem', fontWeight: 600
                                                        }}
                                                    />
                                                </TableCell>
                                                <TableCell align="center" sx={cellStyle}>
                                                    {lesson.needs_computers ? <Computer size={16} color="#3b82f6" /> : <Typography sx={{ color: isDark ? 'rgba(255,255,255,0.2)' : '#cbd5e1' }}>-</Typography>}
                                                </TableCell>
                                                <TableCell align="center" sx={cellStyle}>
                                                    {lesson.status === 2
            ...
            >
                <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1 }}>סדר עדיפויות הקצאה אוטומטית:</Typography>
                <Box component="ol" sx={{ pl: 2, m: 0, fontSize: "0.875rem", lineHeight: 1.6 }}>
                    <li>שימור כיתות לצוות - צוות שקיבל כיתה מסוימת ישאר איתה.</li>
                    <li>שימור כיתות לפלוגה - העדפה לאותה כיתה שהפלוגה השתמשה בה.</li>
                    <li>שימור אזור - העדפה לכיתות באותו אזור/בניין.</li>
