...
const KeysAllocator = () => {
  ...
  const toggleSelectAll = () => {
    setSelectedKeys(
      selectedKeys.length === allKeys.length ? [] : allKeys.map((k) => k.id),
    );
  };
  ...
  const allocateKeys = async () => {
    ...
    if (previousDayLessons) {
      for (const lesson of previousDayLessons) {
        ...
        previousKeyMap[isPlatoon ? `platoon_${groupId}` : `team_${groupId}`] =
          lesson.room_number;
      }
    }
    ...
    const findUserGdud = async (groupId) => {
      ...
      for (const key of availableKeys) {
        ...
        const sessionOverlap = sessionAllocations.some(
          (alloc) =>
        ...
        if (previousKeyMap[`team_${lesson.team_id}`] === key.room_number)
          score += 5000;
        if (
    ...
    for (const up of finalUpdates) {
      await supabase
        .from("schedule_lessons")
        .update({ room_number: up.room_number, status: 2 })
        .eq("id", up.id);
    }
  ...
  const resetAllocations = async () => {
    if (window.confirm("האם למחוק את כל ההקצאות?")) {
      await supabase
        .from("schedule_lessons")
        .update({ room_number: null, status: 1 })
        .eq("date", selectedDate)
        .neq("status", 1);
      setRefreshTrigger((prev) => prev + 1);
    }
  ...
  const handleManualAssign = async (lessonId, roomNumber) => {
    ...
    await supabase
      ...
      .update({
        ...
        status: roomNumber === "unassign" ? 1 : 2,
      })
      .eq("id", lessonId);
    setLessons((prev) =>
      prev.map((l) =>
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Typography
            variant="h5"
            sx={{ fontWeight: 700, color: isDark ? "white" : "#1e293b" }}
          >
            אין הרשאת גישה
          </Typography>
        </Card>
    ...
    <Container maxWidth="xl" sx={{ py: 6 }}>
        ...
        >
          <TextField
            ...
            InputProps={{
              ...
              style: { color: isDark ? "white" : "inherit", fontWeight: 500 },
            }}
          />
        </Box>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        {[
          {
            label: "שובצו",
            value: `${assignedCount}/${lessons.length}`,
            color: "#10b981",
          ...
          {
            label: "שיעורים נבחרים",
            value: selectedLessons.length,
            color: "#8b5cf6",
          ...
          <Grid item xs={12} sm={4} key={i}>
            ...
            >
              ...
              >
                ...
                <Typography
                  variant="h4"
                  sx={{ color: isDark ? "white" : "#1e293b", fontWeight: 700 }}
                >
                  {stat.value}
                </Typography>
              </Card>
          ...
          >
            ...
            >
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <Typography
                  variant="h6"
                  sx={{ color: isDark ? "white" : "#1e293b", fontWeight: 600 }}
                >
                  מפתחות זמינים
                </Typography>
                  ...
                  {isLoading ? (
                    ...
                    allItemsToDisplay.map((lesson) => (
                      ...
                      >
                        ...
                        <TableCell
                          align="center"
                          sx={{ ...cellStyle, fontFamily: "monospace" }}
                        >
